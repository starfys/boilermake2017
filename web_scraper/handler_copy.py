...
class MarkovBot(object):
    def __init__(self, hostname='localhost', port=6379):
        ...
        self.max_words = 1000
        self.alpha = 50 #weight multiplier for vital data
    ...
    def train_model(self,type_='init'):
        if type_ == 'init':
            fileLoc = 'data'
            alpha = 1
        else:
            fileLoc = 'userdata'
            alpha = self.alpha
        for i, json_file in enumerate(listdir(fileLoc)):
            print(i)
            try:
                hackathon = json.load(open(fileLoc+'/'+json_file,'r'))
            except:
                print('failed to open')
                continue
            for weightRepetitions in range(alpha):
...
#Trains
mb.train_model()
#Trains on user inputted data
mb.train_model('user stuff')
#Generates
mb.generate_project()

...
import random
from MarkovMongo.MarkovMongo import MarkovMongo
...
class MarkovBot(object):
    ...
    def train_mongo_model(self):
        for i, json_file in enumerate(listdir('data')[:5]):
            print(i)
            try:
            ...
            for project in hackathon:
                ...
                with open('tmp.txt','w') as tmpfile:
                    tmpfile.write(' '.join(words))
                self.mm.insertwords('tmp.txt')
                if i > 5: break

    def generate_from_mongo(self):
...
mb = MarkovBot()
#mb.train_mongo_model()
print(mb.generate_from_mongo())
#Trains
#mb.train_model()
#Generates
#mb.generate_project()
